...
An Android application runs in an isolated virtual machine on the phone's OS, meaning each app runs in isolation to other apps and can only access it's own files. Each application is a distinct user in the Linux system.

An application is made up of 4 components: Activity, Service, BroadcastReceiver, and ContentProvider. Activity can be considered as the foreground (the screens) and Service can be considered as background tasks (e.g. playing music). Then, BroadcastReceiver allow apps to listen to system events, such as low battery or screen locked. Finally, ContentProvider provides data to the app, such as contacts from the user's phone - the app can query and modify this data if allowed.

In most cases, an Android application runs in it's own Linux process, which is important to understand. A process:  (1) tells the system what this application can do, or (2) what other applications can do with this application. (2) exposes functionality to other applications.

## AndroidManifest.xml

Defines all the components (e.g. activities) used in the app to Android build tools, the OS, and Google Play. Includes permissions, intents, API levels, and hardware features.

### Services

Defines either long running tasks or functionality that other apps can use. Service's can use the app's process or it's own process.

There are two types of services: started services and bound services.

> Imagine downloading a playlist on Spotify. You may minimise the app (activity disappears) but the service (download) still continues.

Started Services tell the system to keep them running until their work is complete.
Bound Services are started by other applications or the system itself. Essentially it tells the OS that there exists a relationship between two services, and can therefore infer information (such as importance) to that service.
